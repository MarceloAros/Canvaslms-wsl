...
Param (
    ...
    # Don't prompt for user password when running command with sudo
    [Parameter(
        Mandatory = $false,
        ValueFromPipeline = $false,
        ValueFromRemainingArguments = $false,
...
process {
    ...
    if ($pscmdlet.ShouldProcess("Target", "Operation")) {
        ...
        if ($CREATE_USER) {
            ...
            # Setting sudo no password
            # TODO: TEST THIS
            wsl -d $OUTPUT_DISTRONAME /bin/bash -c "echo '$CREATE_USER_USERNAME ALL=(ALL:ALL) NOPASSWD: ALL' | sudo tee '/etc/sudoers.d/dont-prompt-$CREATE_USER_USERNAME-for-sudo-password'"
